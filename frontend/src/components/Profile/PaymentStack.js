...
const PaymentBtn = ({ name, id, inputTitle, placeHolder, value, dNone }) => {
    ...
    return (
        <View className="w-[100%] justify-center">
            ...
            <TouchableOpacity className="mx-5 mt-2" onPress={amountOfBudget != null && amountOfBudget > 0 ? () => setPayment(true) : () => alert("monto invalido")} >
                <LinearGradient
                    colors={["#570E7E", "#AA7BC3"]}
                    className="h-10 rounded-xl w-full  flex flex-row items-center justify-center shadow-xl shadow-[#570E7E]"
                >
                    <Image source={icon_Paystack} />
                    <Text className="font-extrabold text-xs  color-[#F1F1F1] ml-2"
                    >INGRESAR DINERO</Text>
