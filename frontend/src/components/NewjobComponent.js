...
export default function NewjobComponent({ navigation }) {
  ...
  const RegisterValidationSchema = yup.object().shape({
    titulo: yup.string("Ingresa titulo del post").required("*Campo requerido"),
    categoria: yup
      .string("Seleccione una categoria")
      .required("*Campo requerido"),
    presupuesto: yup.number().required("*Campo requerido").positive().integer(),
    descripcion: yup
      .string("Ingresa titulo del post")
      .required("*Campo requerido"),
    fechaLimite: yup.date().default(() => new Date()),
    userId: yup.string(),
  });
  ...
  const postear = async (data) => {
    ...
    try {
      ...
      let toast = Toast.show(
        "Algo salio mal, intente nuevamente o comuniquese con el administrador",
        {
          duration: Toast.durations.LONG,
