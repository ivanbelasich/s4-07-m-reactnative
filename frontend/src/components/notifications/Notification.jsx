...
export default function Notification({ item }) {
    ...
    return (
        ...
        <TouchableOpacity>
            <View className={`mt-2 px-5 flex-row  items-center relative h-[98px] ${
                    !item.leido ? "bg-light-purple" : "bg-[#D9C5E3]"
                }`}>
                <Text className='pr-3 pt-2 text-md absolute right-0 top-0'>{item.fecha?.slice(0, 10)}</Text>
                ...
                <View className="flex-row  items-center" >
                    ...
                    <View className="ml-5">
                        <Text className='text-lg text-dark-purple font-extrabold'>¡{item.descripcion}!</Text>
                        <Text className='text-base'>Tu postulación para {item.titulo} fue aceptada</Text>
                    </View>
                </View>
